...
function handleConfirm() {
  ...
  setTimeout(() => {
    document
      .getElementById("confirmButtonTrigger")
      .addEventListener("click", deleteTriggerActivity);
  });
}
...
function handleCityClick(e) {
  ...
  setTimeout(() => {
    document
      .getElementById("confirmButtonTrigger")
      .addEventListener("click", deleteTriggerCity);
  });
}
...
function editItinerary(passObj) {
  ...
  setTimeout(() => {
    document
      .getElementById("editActivity")
      .setAttribute("value", passObj.schedule.activity);
...
function getRecommendations() {
  ...
  if (startDate) {
    var recommededTrip =
      recommendedTrips[Math.round(Math.random() * recommendedTrips.length)];
...
function submitEdit() {
  ...
  var securityCheck =
    keepNumbersOnly(endTime) > keepNumbersOnly(startTime) &&
  ...
  setTimeout(() => {
    if (securityCheck) {
      ...
      swal(
        "end time can not be earlier than start time! Activity can not be empty!"
      );
    }
  });
}
