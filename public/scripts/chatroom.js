...
if (!groupID) {
  swal("please join a travel group to start chat!").then(() => {
...
function insertMessage(msg, userName, time, email, msgName) {
  ...
  if (typeof msg == "string") {
    ...
    messageCard.querySelector(
      ".chatMessageText"
    ).innerHTML = `<img class="clientImage" src="data:image/png;base64,${msg.imageData}" />`;
  }
...
if (groupID) {
  ...
  socket.on("chatMessage", async ({ chatMessageObj }) => {
    ...
    insertMessage(
      ...
      chatMessageObj.userName,
      chatMessageObj.timeStp,
      chatMessageObj.email,
      chatMessageObj.messageName
    );
  });
...
chatRoomView.addEventListener("click", function (event) {
  ...
  if (event.target.classList.contains("delete-button")) {
    ...
    const messagerNameElement =
      chatMessageBoxTwo.querySelector(".messagerName");
    ...
    const chatMessageTextElement =
      chatMessageBoxTwo.querySelector(".chatMessageText");
  ...
  for (var i = 0; i < elems.length; i++) {
    if (i == elems.length - 1) {
      ...
      elems[i].setAttribute(
        "class",
        `${memberSentiment.email}emoji emojiIcon emojiAnimate`
      );
    } else {
      elems[i].innerHTML = "";
